...
function initSlots(facility) {
    ...
    bookBtn.addEventListener('click', () => {
        ...
        // Add Teammate recruitment options
        const recInput = document.getElementById('teammateRequired');
        if (recInput) {
            localStorage.setItem('booking_rec', recInput.value);
            localStorage.setItem('booking_rec_players', document.getElementById('playersNeeded').value);
        }
...
/**
 * Toggle for Need Teammates
 */
function setTeammateRequired(choice) {
    const noBtn = document.getElementById('recNo');
    const yesBtn = document.getElementById('recYes');
    const input = document.getElementById('teammateRequired');
    const countDiv = document.getElementById('recPlayerCount');

    if (!input) return;
    input.value = choice;

    if (choice) {
        yesBtn.style.background = '#8e44ad';
        yesBtn.style.color = 'white';
        noBtn.style.background = 'transparent';
        noBtn.style.color = '#666';
        countDiv.style.display = 'flex';
    } else {
        noBtn.style.background = '#8e44ad';
        noBtn.style.color = 'white';
        yesBtn.style.background = 'transparent';
        yesBtn.style.color = '#666';
